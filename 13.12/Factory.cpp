...
{
	PointLight* tmp = new PointLight(defaultLight);

	Scene* result1 = new Scene((Light*)tmp);

	
	PointLight* pl = new PointLight(glm::vec3(5.0f, 5.0f, 0.0f));
	DirectionalLight* dl = new DirectionalLight(glm::vec3(0.0f, 0.0f, 5.0f));
	SpotLight* sl = new SpotLight(result1->GetCameraPosition(), result1->GetCameraPosition());


	//Scene* result = new Scene(new Light(defaultLight));
	Scene* result = new Scene((Light*)sl);
...
Object* Factory::CreateTexturePlain(FactoryParams params, glm::mat4 matrix, const float points[], int size)
{
	...
	ShaderProgram*	sp = new ShaderProgram("lights.vert", "lights.frag", params.scene->ReturnCamera(), params.scene->ReturnLight());

	//return new Object(gm, sp, glm::scale(glm::translate(matrix, glm::vec3(0.0f, -10.0f, 0.0f)), glm::vec3(10.0f, 10.0f, 10.0f)), "test.png");

	return new Object(gm, sp, true, glm::scale(glm::translate(matrix, glm::vec3(0.0f, -10.0f, 0.0f)), glm::vec3(10.0f, 10.0f, 10.0f)));
}
