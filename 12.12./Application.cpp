...
{
	if (num > manager->GetNumberOfElements())
	{
		printf("Cannot set this scene number.");
		return;
	}

	activeScene = manager->GetThScene(num);
}
#pragma endregion
...
void Application::SetUp()
{
	...
	window = glfwCreateWindow(800, 600, "ZPG - hab0065", NULL, NULL);
	if (!window)
	{
		glfwTerminate();
		::exit(EXIT_FAILURE);
...
void Application::Run()
{
	...
	while (!glfwWindowShouldClose(window))
	{
	   ...
	   renderer->RenderScene(activeScene, projMat);
